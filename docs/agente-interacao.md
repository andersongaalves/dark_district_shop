...
O modo legado do WhatsApp é humano, com uma única saudação fixa. Quando o modo híbrido
está ativo, AUTO e ASSIST reutilizam este agente com as mesmas ferramentas de leitura;
consulte [o guia híbrido](whatsapp-hybrid.md).
...
evitando confirmar uma correção fora de contexto. No WhatsApp híbrido, sugestões são
geradas separadamente e somente sob ação explícita na inbox administrativa.

## Publicação e testes

Esta etapa histórica não criou migration, dependência ou troca de modelo. A arquitetura
posterior do WhatsApp híbrido adicionou migration e consumer documentados no guia atual.
